function [ss, HG, H2G, qHG, HGG, ndhCA, SyC, RR]=uv2to1itc(para,initial,DA) %ss is the outcome of this function, it
%%
%               FIT BINDING
%
%
...
htoti=initial(npd+1,2)*1e-3;
Ltoti=initial(npd+1,3)*1e-3;
...
b=uv2to1bbb1(Ltot,htot,K11,K12); 
bi=uv2to1bbb1(Ltoti,htoti,K11,K12); 
...
H2G = htot.*(((b.*b.*K11.*K12))./(1+(b.*K11)+(b.*b.*K11.*K12)));

HGi = htoti.*((bi.*K11)./(1+(bi.*K11)+(bi.*bi.*K11.*K12)));
H2Gi = htoti.*(((bi.*bi.*K11.*K12))./(1+(bi.*K11)+(bi.*bi.*K11.*K12)));

%HGG=(vzero.*HG.*DH1)+(vzero.*H2G.*(DH1+DH2));
%HGGi=(vzero.*HGi.*DH1)+(vzero.*H2Gi.*(DH1+DH2));

HGG=(vzero.*HG.*DH1)+(vzero.*H2G.*(D2H));
HGGi=(vzero.*HGi.*DH1)+(vzero.*H2Gi.*(D2H));
