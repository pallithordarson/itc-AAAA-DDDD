...
[results, fval, exitflag, output] = fminsearch('uv1to2to1itc',start1,options,initial,DA);
...
%NEXTLINEVARIABLE (Marker for section that is variable between different fitting programs)   
[ss, HG, H2G, HG2, qHG, HGG, ndhCA, SyC, RR]=uv1to2to1itc(para,initial,DA);

%ENDVARIABLE (Marker for end of section that is variable between different fitting programs)

results(1:3)=10.^results(1:3);
oldstart(1:3)=10.^oldstart(1:3);
...
FitConcAndSpectra=struct('Host_Guest_Q_data',[HGG],'calculated_molar_heat',ndhCA,'host_guest_1t1_conc',HG,'heat_per_injection',qHG,'residuals',RR,'SyC',SyC,'host_guest_1t2_conc',HG2,'host_guest_2t1_conc',H2G);
